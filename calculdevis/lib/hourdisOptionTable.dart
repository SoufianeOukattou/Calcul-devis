...
class HourdisOption extends StatefulWidget {
  @override
  _HourdisOptionState createState() => _HourdisOptionState();
}

class _HourdisOptionState extends State<HourdisOption> {
              ...
              onPressed: () async {
                ...
                if (type.isNotEmpty && price > 0) {
                  ...
                  if (existingSettings.docs.isNotEmpty) {
                    ...
                    await FirebaseFirestore.instance.collection('HourdisOption').add({
                      'type': type,
                      'price': price,
                    });
  ...
  void _deleteSetting(String documentId) async {
    await FirebaseFirestore.instance.collection('HourdisOption').doc(documentId).delete();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Hourdis Settings Page'),
      ),
      body: StreamBuilder<QuerySnapshot>(
        stream: FirebaseFirestore.instance.collection('HourdisOption').snapshots(),
